...
namespace EasyOn {
    public partial class ReadForm : Form {
        ...
        public ReadForm(User user, Memo memo) {
            InitializeComponent();

            _user = user;
            _memo = memo;

            string regdateStr = _memo.regdate.ToString("yy-MM-dd HH:mm:ss");
            Text = _memo.writerNickname + " (" + _memo.writerId + ") 님의 쪽지 " + regdateStr;
            titleLabel.Text = _memo.writerNickname + " (" + _memo.writerId + ") 님의 쪽지 " + regdateStr;
            readMsg.Text = _memo.msg;
        }
        ...
        private void sendButton_Click(object sender, EventArgs e) {
            ...
            Worker.getInstance().sendPacket(new C_Memo(C_Memo.TYPE_MSG, _memo.writerNo, msg));
            Close();
        }
    }
}
